...
# Updated v3_pools dictionary with fee included
v3_pools = {
...
def execute_arbitrage(v3_pool_info, is_v2_to_v3, max_amount):
    ...
    try:
        ...
        logging.info(f"Flashloan transaction sent. Hash: {tx_hash.hex()}")

        receipt = w3_exec.eth.wait_for_transaction_receipt(tx_hash)
        logging.info(f"Flashloan transaction mined. Status: {'Success' if receipt['status'] == 1 else 'Failed'}")
        logging.info(f"Flashloan transaction details: Gas used: {receipt['gasUsed']}, Effective gas price: {receipt['effectiveGasPrice']}")
...
def calculate_optimal_flashloan_amount(v3_pool_address, price_difference):
    try:
        ...
        # Ensure we don't exceed maximum sensible amount
        return min(optimal_amount, w3_exec.to_wei(1000, 'ether'))
    except Exception as e:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logging.info("Restarting monitoring in 60 seconds...")
            time.sleep(60)