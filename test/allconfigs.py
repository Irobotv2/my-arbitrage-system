...
def calculate_optimal_flashloan_amount(v3_pool_address, price_difference):
    try:
        ...
        # Convert liquidity to a more manageable number
        max_amount = liquidity / (10 ** 18)
        
        # Calculate optimal amount based on price difference
        optimal_amount = max_amount * (price_difference / 100)
        
        # Ensure we don't exceed maximum sensible amount or 10% of pool liquidity
        return min(optimal_amount, w3_exec.to_wei(1000, 'ether'), max_amount * 0.1)
    except Exception as e:
        main_logger.error(f"Error calculating optimal flashloan amount: {str(e)}")
        return w3_exec.to_wei(0.1, 'ether')  # Return a smaller default amount
...
def execute_arbitrage(config, v3_pool_info, is_v2_to_v3, max_amount):
    ...
    flash_loan_amount = int(max_amount)  # Ensure this is an integer (Wei value)

    # Add approval step
    approve_tokens(sorted_tokens[0], flash_loan_amount)
    approve_tokens(sorted_tokens[1], flash_loan_amount)
    ...
    try:
        ...
        if "SafeMath: subtraction overflow" in str(ve):
            main_logger.error(f"Insufficient token balance: {str(ve)}")
        elif "UniswapV2Library: INSUFFICIENT_INPUT_AMOUNT" in str(ve):
            main_logger.error(f"Insufficient input amount: {str(ve)}")
        else:
            main_logger.error(f"ValueError in execute_arbitrage: {str(ve)}")
    except Exception as e:
        ...
        for config_key, config in configurations.items():
            ...
            for fee_tier, pool_info in config['v3_pools'].items():
                ...
                if price_v2:  # Only check for arbitrage if V2 pool exists
                    # Check for V2 to V3 opportunity
                    if price_v3 > price_v2 * (1 + DETECTION_THRESHOLD):
                        ...
                        if price_v3 > price_v2 * (1 + EXECUTION_THRESHOLD):
                            ...
                            optimal_amount = calculate_optimal_flashloan_amount(pool_info['address'], price_difference)
                            execute_arbitrage(config, pool_info, is_v2_to_v3=True, max_amount=optimal_amount)

                    # Check for V3 to V2 opportunity
                        ...
                        if price_v2 > price_v3 * (1 + EXECUTION_THRESHOLD):
                            ...
                            optimal_amount = calculate_optimal_flashloan_amount(pool_info['address'], price_difference)
                            execute_arbitrage(config, pool_info, is_v2_to_v3=False, max_amount=optimal_amount)
