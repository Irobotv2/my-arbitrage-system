...
async def scan_price_deviations():
    ...
    async def process_token(token_key):
        token_data = redis_client.hgetall(token_key)
        ...
        prices = []
        tasks = [get_pool_price(None, redis_client.hgetall(pool_key)['pair_address'] if 'pair_address' in redis_client.hgetall(pool_key) else redis_client.hgetall(pool_key)['pool_address'], 'v3_pool' in pool_key) for pool_key in all_pools]
        pool_prices = await asyncio.gather(*tasks)
        
        prices = [(price, pool_key) for price, pool_key in zip(pool_prices, all_pools) if price is not None]
    ...
    tasks = [process_token(token_key) for token_key in all_tokens]
...
# Assuming these constants are defined elsewhere in your script
MIN_PRICE = Decimal('1e-12')
...
def verify_liquidity_imbalance(pool_data):
    try:
        ...
        if imbalance > 1:  # More than 100% imbalance is suspicious
            logger.warning(f"Unusually high imbalance detected: {imbalance}")
...
async def get_pool_price(session, pool_address, is_v3):
    try:
        ...
        if price < MIN_PRICE or price > MAX_PRICE:
            logger.warning(f"Unusual price detected for pool {pool_address}: {price}")
            return None
...
async def scan_liquidity_imbalances():
    ...
    for pool_key in all_pools:
        ...
        if imbalance is not None and imbalance > LIQUIDITY_IMBALANCE_THRESHOLD:
            ...
            logger.info(f"Imbalance detected: {imbalance}")
            logger.info(f"Pool data: {pool_data}")
    
    return opportunities

def verify_profit_calculation(simulated_profit, flash_loan_amount):
    profit = Decimal(Web3.from_wei(simulated_profit, 'ether'))
    flash_loan_amount_eth = Decimal(Web3.from_wei(flash_loan_amount, 'ether'))
    
    if flash_loan_amount_eth == 0:
        return None
...
async def simulate_arbitrage(path, flash_loan_amount):
    ...
    try:
        ...
        profit_percentage = verify_profit_calculation(estimated_profit, flash_loan_amount)
        
        if profit_percentage is None:
            return {'success': False, 'error': 'Unusually high profit'}
        
        logger.info(f"Simulated profit: {Web3.from_wei(estimated_profit, 'ether')} WETH ({profit_percentage:.2f}%)")
        return {
            'success': True,
            'profit': Web3.from_wei(estimated_profit, 'ether'),
            'path': swap_steps,
...
async def execute_arbitrage_on_tenderly(path, flash_loan_amount):
    ...
    try:
        ...
        for token_in, token_out, pool in path:
            ...
            if price is None:
                logger.error(f"Invalid price for {get_token_symbol(token_in)} -> {get_token_symbol(token_out)}")
...
async def handle_opportunity(opportunity, opportunity_type):
    ...
    if success:
        logger.info(f"Successfully executed {opportunity_type} arbitrage. Actual Profit: {Web3.from_wei(actual_profit, 'ether')} WETH")
        return True, actual_profit, tx_receipt
    else:
        logger.warning(f"Failed to execute {opportunity_type} arbitrage on Tenderly.")
        return False, 0, tx_receipt
...
async def handle_price_deviation(opportunity):
    ...
    success, tx_receipt, actual_profit = await execute_arbitrage_on_tenderly(path, FLASH_LOAN_AMOUNT)
    if success:
        logger.info(f"Successfully executed price deviation arbitrage. Actual Profit: {Web3.from_wei(actual_profit, 'ether')} WETH")
        return True, actual_profit, tx_receipt
...
async def handle_liquidity_imbalance(opportunity):
    ...
    success, tx_receipt, actual_profit = await execute_arbitrage_on_tenderly(path, FLASH_LOAN_AMOUNT)
    if success:
        logger.info(f"Successfully executed liquidity imbalance arbitrage. Actual Profit: {Web3.from_wei(actual_profit, 'ether')} WETH")
        return True, actual_profit, tx_receipt
...
async def main_loop(all_opportunities, executed_opportunities):
    try:
        ...
        for idx, opp in enumerate(price_opportunities, 1):
            ...
            success, actual_profit, receipt = await handle_opportunity(opp, 'price_deviation')
            all_opportunities.append({**opp, 'success': success, 'actual_profit': actual_profit, 'receipt': receipt})
            if success:
                executed_opportunities.append({**opp, 'actual_profit': actual_profit, 'receipt': receipt})
        ...
        for idx, opp in enumerate(imbalance_opportunities, 1):
            ...
            success, actual_profit, receipt = await handle_opportunity(opp, 'liquidity_imbalance')
            all_opportunities.append({**opp, 'success': success, 'actual_profit': actual_profit, 'receipt': receipt})
            if success:
                executed_opportunities.append({**opp, 'actual_profit': actual_profit, 'receipt': receipt})
