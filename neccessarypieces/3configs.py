...
def load_configurations_from_redis():
    """
    Load all configurations from Redis and populate the pool graph.
    """
...
def generate_arbitrage_paths(graph):
    """
    Generate potential arbitrage paths for all token pairs.
    """
    paths = []

    # Iterate over all tokens in the graph
    for start_token_address in graph.tokens.keys():
        start_token = graph.tokens[start_token_address]

        # Generate paths for each token
...
def format_path(path):
    """
    Formats the path for simpler logging.
    """
    result = ["Path:"]
...
def calculate_execution_price(amount_in, token_in, token_out, pair_contract):
    # Fetch the correct addresses of token0 and token1 from the contract
    token0_address = pair_contract.functions.token0().call()
    token1_address = pair_contract.functions.token1().call()

    # Get reserves dynamically based on token addresses
    ...
    # Calculate amount out based on the input token's reserve
    ...
    # Convert the output back to the appropriate decimals
